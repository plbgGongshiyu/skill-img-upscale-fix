...
DEFAULT_STATUS_ENDPOINT = f"{DEFAULT_BASE_URL}/task/openapi/status"
DEFAULT_OUTPUTS_ENDPOINT = f"{DEFAULT_BASE_URL}/task/openapi/outputs"
...
def _get_status(api_key: str, task_id: str, status_url: str) -> Dict[str, Any]:
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    }
    body = {"apiKey": api_key, "taskId": task_id}
    response = requests.post(status_url, headers=headers, json=body, timeout=120)
    response.raise_for_status()
    return response.json()


def _get_outputs(api_key: str, task_id: str, outputs_url: str) -> Dict[str, Any]:
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    }
    body = {"apiKey": api_key, "taskId": task_id}
    response = requests.post(outputs_url, headers=headers, json=body, timeout=120)
    response.raise_for_status()
    return response.json()
...
def main() -> None:
    ...
    parser.add_argument("--status-url", default=DEFAULT_STATUS_ENDPOINT)
    parser.add_argument("--outputs-url", default=DEFAULT_OUTPUTS_ENDPOINT)
    ...
    parser.add_argument("--timeout", type=int, default=1800, help="Max seconds to wait")
    ...
    status = None
    while True:
        status_json = _get_status(args.api_key, task_id, args.status_url)
        status = _extract_task_status(status_json)
    ...
    outputs_json = _get_outputs(args.api_key, task_id, args.outputs_url)

    print(json.dumps({"taskId": task_id, "status": status, "outputs": outputs_json}, ensure_ascii=True))
